...
class Receipt extends CI_Controller {
    ...
    public function toPdf() {
        ...
        $data = $this->data_model->get_invoice('a.isDelete=0 and a.id='.$id.' and a.billType="RECEIPT"',1);
        if (count($data)>0) {
            $data['num']    = 8;
            ...
            $count = 0;
            $count2 = 0;
//            var_dump($postData['accounts']);exit;
            foreach ($postData['accounts'] as $arr=>$row){
                $account = $this->data_model->get_account_info('d.id='.$row['accId'].' and c.id='.$row['wayId']);
//                var_dump($account);exit;
                $accounts[$arr]['i']       = $arr + 1;
                $accounts[$arr]['account'] = $account[0]['accountNumber'].$account[0]['accountName'];
                $accounts[$arr]['payment'] = $row['payment'];
                $accounts[$arr]['wayId']   = $account[0]['categoryName'];
                $accounts[$arr]['settlement'] = $row['settlement'];
                $accounts[$arr]['remark']   = $row['remark'];
                $count += $row['payment'];
            }
            foreach ($postData['entries'] as $arr=>$row) {
                $entries[$arr]['i']            = $arr + 1;
                $entries[$arr]['billNo']       = $row['billNo'];
                $entries[$arr]['transType']    = $row['transType'];
                $entries[$arr]['billDate']     = $row['billDate'];
                $entries[$arr]['billPrice']    = $row['billPrice'];
                $entries[$arr]['hasCheck']     = $row['hasCheck'];
                $entries[$arr]['notCheck']     = $row['notCheck'];
                $entries[$arr]['nowCheck']     = $row['nowCheck'];
                $count2 += $row['nowCheck'];
            }
            $data['countpage']  = ceil(count($postData['accounts'])/$data['num']);
            $data['list']       = isset($accounts) ? $accounts : array();
            $data['list2']      = isset($entries) ? $entries : array();
            $data['count']      = $count;
            $data['count2']     = $count2;
