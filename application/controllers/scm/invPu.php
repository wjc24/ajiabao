...
class InvPu extends CI_Controller {
    ...
    public function delete() {
		...
		if (count($data)>0) {
		    foreach($data as $arr=>$row) {
			    $row['checked'] >0 && str_alert(-1,'其中已确认的不可删除');
				$ids[]           = $row['id'];
				$billNo[]        = $row['billNo'];
	...
	public function checkInvPu() {
		...
		if (strlen($data)>0) {
			...
			if ($this->db->trans_status() === FALSE) {
			    ...
			    $this->db->trans_commit();
				$this->common_model->logs('采购单据编号：'.$data['billNo'].'的单据已确认！');
				str_alert(200,'success',array('id'=>$data['id']));
			}
		}
		str_alert(-1,'提交的数据不能为空');
    ...
    public function batchCheckInvPu() {
		...
		if (count($data)>0) {
		    foreach($data as $arr=>$row) {
			    $row['checked'] > 0 && str_alert(-1,'勾选当中已确认，不可重复已确认');
			    $ids[]        = $row['id'];
				$billNo[]     = $row['billNo'];
			...
			if ($sql) {
				...
				$this->common_model->logs('购货单编号：'.$billNo.'的单据已确认！');
				str_alert(200,'单据编号：'.$billNo.'的单据已确认！');
			}
			str_alert(-1,'已确认失败');
		}
		str_alert(-1,'单据不存在！');
    ...
    public function rsBatchCheckInvPu() {
		...
		if (count($data)>0) {
		    foreach($data as $arr=>$row) {
			    $row['checked'] < 1 && str_alert(-1,'勾选当中已有可修改，不可重复可修改');
				$ids[]        = $row['id'];
				$billNo[]     = $row['billNo'];
				$srcOrderId[] = $row['srcOrderId'];
			}
			...
			if ($sql) {
				...
				$this->common_model->logs('购货单单号：'.$billNo.'的单据已被可修改！');
				str_alert(200,'购货单编号：'.$billNo.'的单据已被可修改！');
			}
			str_alert(-1,'可修改失败');
		}
		str_alert(-1,'单据不存在！');
