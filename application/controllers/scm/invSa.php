	...
	public function index() {
		...
		switch ($action) {
			...
			case 'editSale':
                ...
                $data['amountType'] = $this->mysql_model->get_row('invoice',array('id'=>$id,'billType'=>'SALE'),'amountType');
                $data['rate'] = $this->db->order_by('id DESC')->get('ci_rate')->row()->taxRate;
			    $this->load->view('scm/invSa/initSale',$data);
				break;
			case 'initUnhxList':
	...
	public function add(){
		...
		if (strlen($data)>0) {
			...
			$info = elements(array(
				...
				'salesId','uid','userName','accId','modifyTime','udf01','udf02','udf03','totalRateAmount','amountType','totalBeforeAmount'),$data,NULL);
			$this->db->trans_begin();
	...
	public function updateInvSa(){
		...
		if (strlen($data)>0) {
		    ...
		    $info = elements(array(
				...
				'salesId','accId','modifyTime','udf01','udf02','udf03','totalRateAmount','amountType','totalBeforeAmount'),$data,NULL);
			$this->db->trans_begin();
	...
	public function checkInvSa() {
		...
		if (strlen($data)>0) {
			...
			if ($this->db->trans_status() === FALSE) {
			    ...
			    $this->db->trans_commit();
				$this->common_model->logs('销货单据编号：'.$data['billNo'].'的单据已被已确认！');
				str_alert(200,'success',array('id'=>$data['id']));
			}
		}
	...
	public function revsCheckInvSa() {
		...
		if (strlen($data)>0) {
			...
			if ($sql) {
				...
				$this->common_model->logs('购货单据编号：'.$data['billNo'].'的单据已被可修改！');
				str_alert(200,'success',array('id'=>$data['id']));
			}
			str_alert(-1,'SQL错误');
		}
    ...
    public function batchCheckInvSa() {
		...
		if (count($data)>0) {
			...
			if ($sql) {
				...
				$this->common_model->logs('销货单编号：'.$billno.'的单据已确认！');
				str_alert(200,'销货单编号：'.$billno.'的单据已确认！');
			}
			str_alert(-1,'已确认失败');
		}
		str_alert(-1,'所选的单据都已确认，请选择可修改的销货单进行已确认！');
	}
    ...
    public function rsBatchCheckInvSa() {
		...
		if (count($data)>0) {
			...
			if ($sql) {
				...
				$this->common_model->logs('销货单：'.$billno.'的单据已被可修改！');
				str_alert(200,'销货单编号：'.$billno.'的单据已被可修改！');
			}
			str_alert(-1,'可修改失败');
		}
		str_alert(-1,'所选的销货单都是可修改，请选择已确认的销货单进行可修改！');
	}
