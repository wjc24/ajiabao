...
class InvSo extends CI_Controller {
    ...
    public function delete() {
		...
		if (count($data)>0) {
		    $data['checked'] >0 && str_alert(-1,'已确认的不可删除');
		    $this->db->trans_begin();
    ...
    public function batchcheckinvSo() {
		...
		if (count($data)>0) {
		    $this->mysql_model->get_count('invoice','(srcOrderId in('.$id.')) and isDelete=0')>0 && str_alert(-1,'有关联的销货单，不能对它进行已确认！');
			...
			if ($sql) {
				...
				$this->common_model->logs('销货订单编号：'.$billno.'的单据已被已确认！');
				str_alert(200,'订单编号：'.$billno.'的单据已被已确认！');
			}
			str_alert(-1,'已确认失败');
		}
		str_alert(-1,'所选的单据都已被已确认，请选择可修改的单据进行已确认！');
	}
    ...
    public function rsbatchcheckinvSo() {
		...
		if (count($data)>0) {
		    $this->mysql_model->get_count('invoice','(srcOrderId in('.$id.')) and isDelete=0')>0 && str_alert(-1,'有关联的销货单，不能对它进行可修改！');
			$sql = $this->mysql_model->update('order',array('checked'=>0,'checkName'=>''),'(id in('.$id.'))');
			if ($sql) {
				$billno = array_column($data,'billNo','id');
				$billno = join(',',$billno);
				$this->common_model->logs('销货订单：'.$billno.'的单据已被可修改！');
				str_alert(200,'订单编号：'.$billno.'的单据已被可修改！');
			}
			str_alert(-1,'可修改失败');
		}
		str_alert(-1,'所选的订单都是可修改，请选择已确认的订单进行可修改！');
	}
	...
	public function checkInvSo() {
		...
		if (strlen($data)>0) {
			$data = $this->validform((array)json_decode($data, true));
			$this->mysql_model->get_count('invoice',array('srcOrderId'=>$data['id'],'isDelete'=>0))>0 && str_alert(-1,'有关联的销货单，不能对它进行可修改！');
			$data['checked']         = 1;
			...
			if ($this->db->trans_status() === FALSE) {
			    ...
			    $this->db->trans_commit();
				$this->common_model->logs('销货单编号：'.$data['billNo'].'的单据已确认！');
				str_alert(200,'success',array('id'=>$data['id']));
			}
		}
		str_alert(-1,'已确认失败');
	...
	public function revsCheckInvSo() {
		...
		if (strlen($data)>0) {
			...
			if ($sql) {
				$this->common_model->logs('销货单号：'.$data['billNo'].'的单据已被可修改！');
				str_alert(200,'success',array('id'=>$data['id']));
			}
		}
		str_alert(-1,'提交失败');
