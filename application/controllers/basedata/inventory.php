...
class Inventory extends CI_Controller {
	...
	public function index() {
		...
		$where = '(a.isDelete=0)';
		$where .= $skey ? ' and (sonGoods like "%'.$skey.'%" or name like "%'.$skey.'%" or number like "%'.$skey.'%" or brand like "%'.$skey.'%" or spec like "%'.$skey.'%")' : '';
	...
	public function add(){
		...
		if ($data) {
			...
			$this->db->trans_begin();
			$info = array(
			    'barCode','baseUnitId','unitName','brand','categoryId','categoryName','propertys',
	...
	public function exporter() {
		...
		$barCode      = intval($this->input->get_post('barCode',TRUE));
		$where = '(a.isDelete=0)';
		$where .= $skey ? ' and (name like "%'.$skey.'%" or number like "%'.$skey.'%" or brand like "%'.$skey.'%" or spec like "%'.$skey.'%")' : '';
		$where .= $barCode ? ' and barCode="'.$barCode.'"' : '';
		if ($categoryid > 0) {
