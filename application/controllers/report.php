...
class Report extends CI_Controller {
	...
	public function puDetail_detail() {
	    ...
	    $brandNo  = str_enhtml($this->input->get_post('brandNo',TRUE));
		...
		$where .= $brandNo ? ' and b.brand in('.str_quote($brandNo).')' : '';
		...
		if ($status){
            if ($status == '未付款'){
                $where .= ' and f.rpAmount + ifnull(g.nowCheck,0) <= 0';
            }else if ($status == '部分付款'){
                $where .= ' and abs(f.rpAmount + ifnull(g.nowCheck,0))<abs(f.amount) and abs(f.rpAmount + ifnull(g.nowCheck,0))>0 ';
            }else if ($status == '全部付款'){
                $where .= ' and abs(f.rpAmount + ifnull(g.nowCheck,0)) >= abs(f.amount)';
            }
        }
		...
		foreach ($list as $arr=>$row) {
		    ...
		    if ($hasCheck <= 0){
                $v[$arr]['status']    = '未付款';
            }else if($hasCheck >= (float)abs($row['Famount'])){
                $v[$arr]['status']    = '全部付款';
            }else{
                $v[$arr]['status']    = '部分付款';
            }
	...
	public function goods_balance() {
		...
		$this->load->view('report/goods-balance',$data);
	}
	...
	public function invBalance() {
		...
		$where .= $brandNo ? ' and b.brand="'.$brandNo.'"' : '';
	...
	public function deliverDetail() {
		...
		$where2 .= $brandNo ? ' and b.brand in('.str_quote($brandNo).')' : '';
