...
class Data_model extends CI_Model{
	...
	public function get_inventory($where='',$type=2) {
	    $sql = 'select 
		            ...
		            b.name as invName,b.brand as invBrand, b.number as invNumber,b.spec as invSpec,b.categoryId ,b.categoryName,b.unitName,b.unitid,
		        ...
		        from '.$this->db->dbprefix('invoice_info').' as a 
					left join 
						(select id,name,brand,number,spec,unitName,unitid,lowQty,highQty,categoryId,categoryName from '.$this->db->dbprefix('goods').' where isDelete=0) as b 
					on a.invId=b.id 
					left join 
	...
	public function get_order_info($where='',$type=2) {
	    $sql = 'select 
		            a.*, 
					b.name as invName,b.brand as invBrand, b.number as invNumber, b.spec as invSpec, 
				...
				from '.$this->db->dbprefix('order_info').' as a 
					left join 
						(select id,name,brand,number,spec,unitName,unitId,pinYin,purPrice,quantity,salePrice from '.$this->db->dbprefix('goods').' where isDelete=0) as b 
					on a.invId=b.id 
					left join 
	...
	public function get_invoice_info($where='',$type=2) {
	    $sql = 'select 
					...
					d.name as locationName ,d.locationNo ,
					e.number as salesNo ,e.name as salesName,
					f.hxStateCode , f.id , f.amount as Famount, f.rpAmount,
					(f.rpAmount + ifnull(g.nowCheck,0)) as hasCheck 
				from '.$this->db->dbprefix('invoice_info').' as a 
				    left join 
						(select hxStateCode,id,amount,rpAmount from '.$this->db->dbprefix('invoice').' where isDelete=0) as f
					on a.iid=f.id 
					left join 
						(select id,name,brand,number,spec,unitName,unitId,pinYin,purPrice,quantity,salePrice from '.$this->db->dbprefix('goods').' where isDelete=0) as b 
					on a.invId=b.id 
					...
					on a.salesId=e.id 
                    left join 
                        (select billId,sum(nowCheck) as nowCheck from '.$this->db->dbprefix('verifica_info').' where isDelete=0 group by billId) as g
                    on a.iid=g.billId
				where '.$where;
		return $this->mysql_model->query($sql,$type);
	}
	...
	public function get_invBalance($where='',$select='',$type=2) {
	    $sql = 'select 
					...
					'.$select.'
					b.name as invName,b.brand as invBrand, b.number as invNumber, b.spec as invSpec, b.unitName as mainUnit, b.categoryId,b.salePrice,
					c.locationNo
				from '.$this->db->dbprefix('invoice_info').' as a 
					left join 
						(select id,name,brand,number,spec,unitName,categoryId,salePrice from '.$this->db->dbprefix('goods').' where isDelete=0) as b 
					on a.invId=b.id 
					left join 
						(select id,name,locationNo from '.$this->db->dbprefix('storage').' where isDelete=0) as c 
					on a.locationId=c.id 
	...
	public function get_deliverSummary($where='',$beginDate,$endDate,$type=2) {
	    $sql = 'select 
					...
					b.name as invName,b.brand as invBrand, b.number as invNumber, b.spec as invSpec, b.unitName as mainUnit,
					c.name as locationName ,c.locationNo
				from '.$this->db->dbprefix('invoice_info').' as a 
				    left join 
						(select id,name,brand,number,spec,unitName from '.$this->db->dbprefix('goods').' where isDelete=0) as b 
					on a.invId=b.id 
					left join 
						(select id,name,locationNo from '.$this->db->dbprefix('storage').' where isDelete=0) as c
					on a.locationId=c.id 
