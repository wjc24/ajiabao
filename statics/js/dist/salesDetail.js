define(["jquery", "print"], function(a, b, c) {
	function d() {
		Business.filterCustomer(), Business.filterGoods(), Business.filterStorage(), Business.filterBrand(), Business.filterSaler(), Business.moreFilterEvent(), l("#conditions-trigger").trigger("click"), l("#filter-fromDate").val(n.beginDate || ""), l("#filter-toDate").val(n.endDate || ""), l("#filter-customer input").val(n.customerNo || ""), l("#filter-goods input").val(n.goodsNo || ""), l("#filter-storage input").val(n.storageNo || ""),l("#filter-brand input").val(n.brandNo || ""), n.beginDate && n.endDate && (l("#selected-period").text(n.beginDate + "至" + n.endDate), l("div.grid-subtitle").text("日期: " + n.beginDate + " 至 " + n.endDate)), l("#filter-fromDate, #filter-toDate").datepicker(), Public.dateCheck();
		var a = parent.SYSTEM;
		...
		l("#filter-submit").on("click", function(a) {
			...
			return c && d && new Date(c).getTime() > new Date(d).getTime() ? void parent.Public.tips({
				...
				brandNo: l("#filter-brand input").val() || "",
				status: l("#filter-status input").val() || "",
				salesId: l("#filter-saler input").val() || "",
				profit: b.chkVal().length > 0 ? "1" : "0"
			}, l("#selected-period").text(c + "至" + d), l("div.grid-subtitle").text("日期: " + c + " 至 " + d), void k(+n.profit)),$("#filter-menu").removeClass("ui-btn-menu-cur")
		}), l("#filter-reset").on("click", function(a) {
			a.preventDefault(), l("#filter-fromDate").val(n.beginDate), l("#filter-toDate").val(n.endDate), l("#filter-customer input").val(""), l("#filter-goods input").val(""), l("#filter-storage input").val(""),l("#filter-brand input").val(""),l("#filter-status input").val(""), l("#filter-saler input").val(""), b.chkNot()
		})
	}
	function e() {
