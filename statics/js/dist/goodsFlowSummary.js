define(["jquery", "print"], function(a) {
	function b() {
		Business.filterGoods(), Business.filterStorage(), Business.filterBrand(), chkboxes = h("#chk-wrap").cssCheckbox(), j.beginDate && j.endDate && h(".grid-subtitle").text(j.beginDate + "至" + j.endDate), i.enableAssistingProp || h("#chk-wrap").hide(), "1" === j.showSku && h('#chk-wrap input[name="showSku"]').attr("checked", !0), h("#filter-fromDate").val(j.beginDate), h("#filter-toDate").val(j.endDate), h("#filter-goods input").val(j.goodsNo), h("#filter-storage input").val(j.storageNo),h("#filter-brand input").val(j.brandNo), Public.dateCheck(), this.fDatePicker = new Pikaday({
	...
	function c() {
		...
		h("#refresh").click(function(b) {
			...
			j = {
				...
				brandNo: h("#filter-brand input").val() || ""
