var curRow, curCol, loading, SYSTEM = system = parent.SYSTEM,
	...
	THISPAGE = {
		...
		initDom: function(a) {
			...
			billRequiredCheck ? (i = '<a class="ui-btn" id="audit">已确认</a>', j = '<a class="ui-btn" id="reAudit">可修改</a>') : (g = l + g, this.$_checkName.parent().hide());
		...
		loadGrid: function(a) {
			...
			function p(a) {
				...
				if (b) {
                    ...
                    f.arrived = f.arrived ? f.arrived : 0;
			...
			for (var a = [], b = $("#grid").jqGrid("getDataIDs"), c = 0, d = b.length; d > c; c++) {
				...
				if ("" !== g.goods) {
                    ...
                    if (this.$_amountType.val() == '2') {
                        g.beforeAmount = g.amount;
                        g.amount = g.rateAmount;
                    }else{
                        g.beforeAmount = g.amount;
                    }
					e = {
