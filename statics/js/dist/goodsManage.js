...
function initField() {
	$("#note").placeholder(), "edit" == oper ? ($("#number").val(rowData.number), $("#name").val(rowData.name), $category.data("defItem", rowData.categoryId), $("#brand").val(rowData.brand), $("#specs").val(rowData.spec), $("#storage").data("defItem", rowData.locationId), $("#unit").data("defItem", ["id", rowData.baseUnitId]), void 0 != rowData.purPrice && $("#purchasePrice").val(Public.numToCurrency(rowData.purPrice, pricePlaces)), void 0 != rowData.salePrice && ($("#salePrice").val(Public.numToCurrency(rowData.salePrice, pricePlaces)),
		$('#rateSalePrice').val((parseFloat(Public.numToCurrency(rowData.salePrice, pricePlaces).replace(/[~'!<>@#$%^&*()-+_=:,]/g, ""))*(parseFloat(rate)) + parseFloat(Public.numToCurrency(rowData.salePrice, pricePlaces).replace(/[~'!<>@#$%^&*()-+_=:,]/g, ""))).toFixed(2)),
		$("#wholesalePrice").val(Public.numToCurrency(rowData.wholesalePrice, pricePlaces)),
		$("#RateWholesalePrice").val((parseFloat(Public.numToCurrency(rowData.wholesalePrice, pricePlaces).replace(/[~'!<>@#$%^&*()-+_=:,]/g, ""))*(parseFloat(rate)) + parseFloat(Public.numToCurrency(rowData.wholesalePrice, pricePlaces).replace(/[~'!<>@#$%^&*()-+_=:,]/g, ""))).toFixed(2)),
