define(["jquery", "print"], function(a, b, c) {
	function d() {
		Business.filterSupplier(), Business.filterGoods(), Business.filterStorage(),Business.filterBrand(), Business.moreFilterEvent(), k("#conditions-trigger").trigger("click"), k("#filter-fromDate").val(m.beginDate || ""), k("#filter-toDate").val(m.endDate || ""), k("#filter-customer input").val(m.customerNo || ""), k("#filter-goods input").val(m.goodsNo || ""),k("#filter-brand input").val(m.brandNo || ""), k("#filter-storage input").val(m.storageNo || ""), m.beginDate && m.endDate && (k("#selected-period").text(m.beginDate + "至" + m.endDate), k("div.grid-subtitle").text("日期: " + m.beginDate + " 至 " + m.endDate)), k("#filter-fromDate, #filter-toDate").datepicker(), k("#filter-submit").on("click", function(a) {
			a.preventDefault();
			...
			return b && c && new Date(b).getTime() > new Date(c).getTime() ? void parent.Public.tips({
                ...
                status: k("#filter-status input").val() || "",
			...
			a.preventDefault(), k("#filter-fromDate").val(m.beginDate), k("#filter-toDate").val(m.endDate), k("#filter-customer input").val(""), k("#filter-goods input").val(""),k("#filter-brand input").val(""),k("#filter-status input").val(""), k("#filter-storage input").val(""), m.customerNo = "", m.goodsNo = "",m.brandNo = "", m.status = "", m.storageNo = ""
		})
	}
	function e() {
