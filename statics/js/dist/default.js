...
function BBSPop() {
	var a = $("#yswb-tab"),
		b = ['<ul id="yswbPop">', '<li class="yswbPop-title">请选择您要进入的论坛</li>', '<li><strong>产品服务论坛</strong><p>在线会计，在线进销存操作问题咨询</p><a href="http://wpa.qq.com/msgrd?V=3&uin=16958398&Site=aerp&Menu=yes" target="_blank">进入论坛>></a></li>', '<li><strong>会计交流论坛</strong><p>会计实操，会计学习，会计资讯</p><a href="http://wpa.qq.com/msgrd?V=3&uin=16958398&Site=aerp&Menu=yes" target="_blank">进入论坛>></a></li>', "</ul>"].join("");
...
//	},
	...
	goodsBrandList: {
		name: "品牌",
		href: "../settings/brand_list?typeNumber=brand",
		dataRight: "TRADETYPE_QUERY",
		target: "setting-auxiliary"
	},
		...
		_initDom: function() {
			if (this.sublist && this.obj) {
				...
				for (var c in a) if (!a[c].disable) {
					...
					if(d.name=="销售利润表"){
						var j = "<li><a " + f + ' tabTxt="' + d.name + '" tabid="' + d.target.split("-")[0] + "-" + c + '" ' + g + ' href="' + d.href + '" data-right="' + d.dataRight + '">' + d.name + h + "" +
							// "<span style='background:url(/statics/css/img/anew.gif) no-repeat 50%;display: inline-block;width: 30px;height: 16px;'></span>" +
							"</a></li>";
...
$("#nav").on("click", "[rel=pageTab]", function(a) {
	...
	if (2 === SYSTEM.siVersion && SYSTEM.isOpen) {
		var b, c = location.protocol + "//" + location.host + "/update_info.jsp",
			d = '您的单据分录已经录入达到300条，继续使用选择<a href="http://wpa.qq.com/msgrd?V=3&uin=16958398&Site=aerp&Menu=yes" target="_blank">购买产品</a>或者完善个人信息赠送1000条免费容量。';
		SYSTEM.isshortUser ? SYSTEM.isshortUser && (b = "http://wpa.qq.com/msgrd?V=3&uin=16958398&Site=aerp&Menu=yes&updateUrl=" + encodeURIComponent(c) + "&warning=" + encodeURIComponent(d) + "&loginPage=http://wpa.qq.com/msgrd?V=3&uin=16958398&Site=aerp&Menu=yes&", a.dialog({
			min: !1,
			max: !1,
			cancle: !1,
			lock: !0,
			...
			title: "完善个人信息",
			content: "url:" + b
		})) : (b = "http://wpa.qq.com/msgrd?V=3&uin=16958398&Site=aerp&Menu=yes&updateUrl=" + encodeURIComponent(c) + "&warning=" + encodeURIComponent(d), a.dialog({
			min: !1,
			max: !1,
			cancle: !1,
			lock: !0,
