define(["jquery", "print"], function(a, b, c) {
	function d() {
		Business.filterSupplier(), Business.filterGoods(), Business.filterStorage(),Business.filterBrand(), Business.moreFilterEvent(), k("#conditions-trigger").trigger("click"), k("#filter-fromDate").val(m.beginDate || ""), k("#filter-toDate").val(m.endDate || ""), k("#filter-customer input").val(m.customerNo || ""), k("#filter-goods input").val(m.goodsNo || ""), k("#filter-storage input").val(m.storageNo || ""),k("#filter-brand input").val(m.brandNo || ""), m.beginDate && m.endDate && (k("#selected-period").text(m.beginDate + "至" + m.endDate), k("div.grid-subtitle").text("日期: " + m.beginDate + " 至 " + m.endDate)), k("#filter-fromDate, #filter-toDate").datepicker(), chkboxes = k("#chk-wrap").cssCheckbox(), k("#filter-submit").on("click", function(a) {
			a.preventDefault();
