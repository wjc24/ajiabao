define(["jquery", "print"], function(a) {
	function b() {
		Business.filterGoods(), Business.filterStorage(),Business.filterBrand(), l.beginDate && l.endDate && j("div.grid-subtitle").text("日期: " + l.beginDate + "至" + l.endDate), j("#filter-fromDate").val(l.beginDate), j("#filter-toDate").val(l.endDate), j("#filter-goods input").val(l.goodsNo), j("#filter-storage input").val(l.storageNo),j("#filter-brand input").val(l.brandNo), Public.dateCheck();
		var a = new Pikaday({
		...
		j("#filter-submit").on("click", function(c) {
			...
			return f.getTime() > g.getTime() ? void parent.Public.tips({
				...
				brandNo: j("#filter-brand input").val() || ""
			}, j("#selected-period").text(d + "至" + e), j("div.grid-subtitle").text("日期: " + d + " 至 " + e), void i())
		}), j("#filter-reset").on("click", function(a) {
			a.preventDefault(), j("#filter-fromDate").val(""), j("#filter-toDate").val(""), j("#filter-goods input").val(""), j("#filter-storage input").val(""),j("#filter-brand input").val("")
		})
	}
	function c() {
