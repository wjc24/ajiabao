	...
	*/ 
	...
	Business.serNumManage = function(opts){
		...
		var oldData = {
			...
			brandInfo : parentTr.data('brandInfo') || {},
			unitInfo : parentTr.data('unitInfo')||{}
		} 
		if(!view){
			//非查询模式
			$grid.jqGrid("restoreCell", curRow, curCol);
			parentTr.data('goodsInfo', null).data('brandInfo',null).data('storageInfo', null).data('unitInfo', null);
		}
		var params = {
			width: 650,
			height: 400,
        ...
        var opts = $.extend(true, {
            ...
            callback: {
                onChange: function(data){
                    ...
                    if(data) {
                        parentTr.data('brandInfo', {id: data.id, name: data.name});
    ...
    Business.filterBrand = function(){
        ...
        $('#filter-brand .ui-icon-ellipsis').on('click', function(){
            ...
            $.dialog({
                ...
                height: 500,
                title: '选择品牌',
                content: 'url:../settings/brand_batch',
                data:{isDelete:2},
                lock: true,
                ok: function(){
                    Business.setFilterBrand(this.content, $input);
                },
                cancel: function(){
                    return true;
                }
    ...
    Business.setFilterBrand = function(dialogCtn, $input){
        for(rowid in dialogCtn.addList){
            var row = dialogCtn.addList[rowid];
        }
        $input.val(row.name);
